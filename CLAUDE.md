...
- **EVC upstream sync** — when `echovaluecalc.com` adds a character (banner fires when `evc_status.acknowledged_date < latest`), pull the rv array + er from the upstream `evc_engine.py` diff (`AstyuteChick/Echo-Value-Calculator`) and **ask the user** for element/weapon/role rather than web-researching. Upstream `er` is now a **per-team dict** `[{team: req_er}, imp, rc]`; our port keeps a single `req_er` = the `"Default"` (or matching split-variant) value, stored **clean** (e.g. `125.0`, not the old `128.1`). Validate any scoring change with a parity harness against `evc_engine.main()`. Detail: `.agent/BACKEND.md → ER surplus-range model`.

- **ER target is a range, not a point** — `scoring_service._init_er_net` applies `_ER_SURPLUS_BUFFER = 3.1`: surplus ≤3.1 ER isn't punished, larger surplus is discounted by 3.1. `req_er < 100` chars zero out the ER substat entirely. Single-echo `calculate_score` delegates to `_score_one_stateful` so both paths share identical ER logic.
